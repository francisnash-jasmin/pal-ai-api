...
crop_dt_pipeline = pickle.load(
    open("./models/crop_recommendation/dt_pipeline.pkl", "rb")
)
crop_lr_pipeline = pickle.load(
    open("./models/crop_recommendation/lr_pipeline.pkl", "rb")
)
...
fertilizer_label_dict = pickle.load(
    open("./models/fertilizer_recommendation/fertilizername_dict.pkl", "rb")
)
soiltype_label_dict = pickle.load(
    open("./models/fertilizer_recommendation/soil_type_dict.pkl", "rb")
)
croptype_label_dict = pickle.load(
    open("./models/fertilizer_recommendation/crop_type_dict.pkl", "rb")
)
...
def crop_prediction(input_data):
    prediction_data = {
        ...
        * 100,
        "dt_model_prediction": crop_label_dict[crop_dt_pipeline.predict(input_data)[0]],
        "dt_model_probability": max(crop_dt_pipeline.predict_proba(input_data)[0])
        * 100,
        "lr_model_prediction": crop_label_dict[crop_lr_pipeline.predict(input_data)[0]],
        "lr_model_probability": max(crop_lr_pipeline.predict_proba(input_data)[0])
        * 100,
    ...
    all_predictions = [
        ...
        prediction_data["dt_model_prediction"],
        prediction_data["lr_model_prediction"],
    ...
    all_probs = [
        ...
        prediction_data["dt_model_probability"],
        prediction_data["lr_model_probability"],
    ]
